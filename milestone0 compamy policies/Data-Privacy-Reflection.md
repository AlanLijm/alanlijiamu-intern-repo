...
- Login credentials are stored securely via trusted providers (Auth0, Zoho).
- Most collected data is only viewed by Focus Bear staff in anonymised/aggregate form — individual details are not shared outside the company.
...
- **Financial Data**: subscription/payment details (handled entirely by Stripe, not stored by Focus Bear directly)
- **Profile & Feedback Data**: individual survey responses, purchase history
...
- Keep login credentials and API keys private — never commit them to a public repository or share them in plain text
- Lock your computer screen when stepping away, especially when working with sensitive data
- Avoid discussing confidential company or user information in public or unsecured settings
...
- Avoid further sharing or discussing the incident outside the appropriate channels
- Follow any additional instructions from the team once reported
...
- Double-check the recipient before sending any file or message containing sensitive information
